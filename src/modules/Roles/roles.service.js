...
import * as dto from "./roles.dto.js";
import * as mapper from "./roles.mapper.js";
...
export async function getAll() {
  return mapper.toRolDtoList(await repo.findAll());
}

export async function getById(id) {
  const rol = await repo.findById(id);
  return rol ? mapper.toRolDto(rol) : null;
}
...
export async function create(rawDto) {
  ...
  return mapper.toRolDto(await repo.create(mapper.fromCreacionDto(parsed)));
}
...
export async function update(id, rawDto) {
  ...
  return mapper.toRolDto(await repo.update(id, mapper.fromModificacionDto(parsed)));
}
