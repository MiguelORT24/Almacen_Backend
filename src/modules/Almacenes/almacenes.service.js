...
import * as dto from "./almacenes.dto.js";
import * as mapper from "./almacenes.mapper.js";
import * as repo from "./almacenes.repository.js";
...
export async function getAll() {
  return mapper.toAlmacenDtoList(await repo.findAll());
}

export async function getById(id) {
  const almacen = await repo.findById(id);
  return almacen ? mapper.toAlmacenDto(almacen) : null;
}
...
export async function create(rawDto) {
  ...
  if (await repo.existsByNombre(parsed.nombre)) {
    throw httpError("Ya existe un almacén con ese nombre", 409);
  }

  return mapper.toAlmacenDto(await repo.create(mapper.fromCreacionDto(parsed)));
}
...
export async function update(id, rawDto) {
  ...
  return mapper.toAlmacenDto(await repo.update(id, mapper.fromModificacionDto(parsed)));
}
